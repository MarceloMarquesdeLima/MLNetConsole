...
namespace MLNetConsole.Exemplo04
{
    class Exemplo05
    {
        public static void Execute()
        {
            ...
            var bestPerformace = allMetrics.OrderByDescending(x => x.RSquared).FirstOrDefault();
            var bestPerformaceIndex = allMetrics.ToList().IndexOf(bestPerformace);
            var bestModels = allModels.ElementAt(bestPerformaceIndex);

            Console.WriteLine("Avarage Data");
